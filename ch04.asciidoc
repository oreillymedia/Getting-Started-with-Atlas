...
Below this paragraph is <<FIG1>> (a figure, titled and cross-referenced). Figures appear exactly where you place them in the text, which can sometimes create pages with a lot of white space. You can add an attribute of `float="true"` to float an image to the top of the page, with the text flowing around it:
