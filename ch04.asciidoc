...
There are two ways to include external code files. You can include an external file that is text-only (no markup like callouts, inline formatting, etc.), or you can include an external file marked up with valid Docbook that can contain these items.

*Text-only External Code File*
...
*External Code File with Inline Markup*

To include an external file that contains inline markup (e.g., for callouts, line annotations, or inline font formatting), use passthrough markup around around the +include::+ macro, instead of code block delimiters:
...
The contents of inline_markup.txt would contain only the Docbook markup. In our example, it might look something like this:
