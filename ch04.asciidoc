...
view the source, available at http://cdn.oreilly.com/atlas_docs.zip[cdn.oreilly.com/atlas_docs.zip].
====
...
====== Fourth-Level Section (Delimited Style) ======
This one is testing a sect4.
...
[WARNING]
====
Please do not use AsciiDoc's mechanisms for forcing line breaks,
...
using a passthrough block (see <<passthroughs>> for an explanation of
passthroughs), like so:
...
The Atlas book-building toolchain supports syntax highlighting via
...
see the full list avaliable on the http://pygments.org/docs/lexers/[the
Pygments website]. Ebook readers that do not have color screens will still
display the highlighting, but in more subtle shades of gray.
